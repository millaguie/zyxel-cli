...
class ZyxelRouter:
    ...
    def login(self):
        ...
        if login_result.get("result") != "ZCFG_SUCCESS":
            print(
                f"Login failed: {login_result.get('result', 'unknown error')}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    def _decrypt_response(self, resp_json):
        ...
        if "iv" not in resp_json or "content" not in resp_json:
            raise ZyxelError(
                ...
                or f"respuesta no cifrada del router: {resp_json}"
            )
...
def cmd_status(router, _args):
    ...
    if ram_total and ram_free:
        ...
        print(
            f"RAM:         {ram_used_pct}% used ({humanize_bytes(ram_free_kb * 1024)} free of {humanize_bytes(ram_total_kb * 1024)})"
        )
    ...
    if lan_ports:
        up_ports = [
            p.get("portName", "?") for p in lan_ports if p.get("status") == "Up"
        ]
        if up_ports:
...
def cmd_dns_add(router, args):
    ...
    for e in entries:
        if e["HostName"] == args.hostname:
            print(
                f"Entry for '{args.hostname}' already exists (#{e['Index']} -> {e['IPv4Address']})"
            )
        ...
        for e in entries:
            if e["HostName"] == args.hostname:
                if router.delete_dns_entry(e["Index"]):
                    print(
                        f"Deleted: {e['HostName']} -> {e['IPv4Address']} (#{e['Index']})"
                    )
                else:
                    print(
                        f"Failed to delete entry for '{args.hostname}'", file=sys.stderr
                    )
                    sys.exit(1)
                return
...
def cmd_dns(router, args):
    {"list": cmd_dns_list, "add": cmd_dns_add, "delete": cmd_dns_delete}[
        args.dns_command
    ](router, args)
...
def cmd_dhcp_list(router, _args):
    ...
    for e in leases:
        ...
        print(
            f"{e['Index']:<{idx_w}}  {e.get('MACAddr', ''):<{mac_w}}  {e.get('IPAddr', ''):<15}  {enabled}"
        )
...
def cmd_dhcp_add(router, args):
    ...
    for e in leases:
        if e.get("MACAddr", "").upper() == mac:
            print(
                f"Lease for {mac} already exists (#{e['Index']} -> {e.get('IPAddr', '?')})"
            )
        ...
        for e in leases:
            if e.get("MACAddr", "").upper() == mac:
                if router.delete_dhcp_lease(e["Index"]):
                    print(
                        f"Deleted: {e['MACAddr']} -> {e.get('IPAddr', '?')} (#{e['Index']})"
                    )
                else:
...
def cmd_dhcp(router, args):
    {"list": cmd_dhcp_list, "add": cmd_dhcp_add, "delete": cmd_dhcp_delete}[
        args.dhcp_command
    ](router, args)
...
def cmd_nat_list(router, _args):
    ...
    print(
        f"{'#':<{idx_w}}  {'Description':<{desc_w}}  {'Proto':<9}  {'External':<11}  {'Internal':<21}  On"
    )
    print(
        f"{'-' * idx_w}  {'-' * desc_w}  {'-' * 9}  {'-' * 11}  {'-' * 21}  {'-' * 3}"
    )
    for e in rules:
        ...
        print(
            f"{e['Index']:<{idx_w}}  {e.get('Description', ''):<{desc_w}}  {proto:<9}  {ext:<11}  {int_full:<21}  {enabled}"
        )
...
def cmd_nat_add(router, args):
    ...
    if router.add_nat_rule(
        args.description, ext_start, ext_end, args.int_ip, int_start, int_end, protocol
    ):
        ext = str(ext_start) if ext_start == ext_end else f"{ext_start}-{ext_end}"
        ...
        for e in rules:
            if e.get("Description", "") == args.description:
                if router.delete_nat_rule(e["Index"]):
                    ...
                    print(
                        f"Failed to delete rule '{args.description}'", file=sys.stderr
                    )
                    sys.exit(1)
                return
...
def cmd_nat(router, args):
    {"list": cmd_nat_list, "add": cmd_nat_add, "delete": cmd_nat_delete}[
        args.nat_command
    ](router, args)
...
def cmd_route_list(router, _args):
    ...
    print(
        f"{'#':<{idx_w}}  {'Name':<{name_w}}  {'Destination':<18}  {'Subnet Mask':<15}  {'Gateway':<15}  On"
    )
    print(
        f"{'-' * idx_w}  {'-' * name_w}  {'-' * 18}  {'-' * 15}  {'-' * 15}  {'-' * 3}"
    )
    for e in routes:
        enabled = "yes" if e.get("Enable") else "no"
        dest = f"{e.get('DestIPAddress', '')}"
        print(
            f"{e['Index']:<{idx_w}}  {e.get('Name', ''):<{name_w}}  {dest:<18}  {e.get('DestSubnetMask', ''):<15}  {e.get('GatewayIPAddress', ''):<15}  {enabled}"
        )
...
def cmd_route_add(router, args):
    ...
    for e in routes:
        if e.get("Name", "") == args.name:
            print(
                f"Route '{args.name}' already exists "
                f"(#{e['Index']} -> {e.get('DestIPAddress')}/{e.get('DestSubnetMask')})"
            )
            print("Delete it first if you want to change it.", file=sys.stderr)
            sys.exit(1)

    if router.add_static_route(
        args.name, args.dest, args.mask, args.gateway, args.interface
    ):
        ...
        for e in routes:
            if e.get("Name", "") == args.name:
                if router.delete_static_route(e["Index"]):
                    print(
                        f"Deleted: {e['Name']} -> {e.get('DestIPAddress')}/{e.get('DestSubnetMask')} (#{e['Index']})"
                    )
                else:
...
def cmd_route(router, args):
    {"list": cmd_route_list, "add": cmd_route_add, "delete": cmd_route_delete}[
        args.route_command
    ](router, args)
...
def cmd_wifi(router, _args):
    ...
    print(
        f"{'#':<3}  {'SSID':<{ssid_w}}  {'Band':<6}  {'Ch':<4}  {'BW':<6}  {'Security':<16}  {'Hidden':<6}  On"
    )
    print(
        f"{'-' * 3}  {'-' * ssid_w}  {'-' * 6}  {'-' * 4}  {'-' * 6}  {'-' * 16}  {'-' * 6}  {'-' * 3}"
    )
    for e in networks:
...
def build_parser():
    ...
    nat_add.add_argument(
        "--ext-port-end", type=int, help="External port end (default: same as start)"
    )
    nat_add.add_argument(
        "--int-port-end", type=int, help="Internal port end (default: same as start)"
    )
    nat_add.add_argument(
        "--proto",
